...
class OutsideController extends Controller
{
    ...
    public function partir(Request $request)
    {
        ...
        $validatedData = $request->validate([
            'form.gain.exp' => 'required|int',
            'form.gain.or' => 'required|int',
            'form.gain.hp_restant' => 'required|int',
        ...
        $user->gold += $validatedData['form']['gain']['or'];
        $user->update();


        $user->heroes[0]->exp_restant += $validatedData['form']['gain']['exp'];
        $user->heroes[0]->hp_restant = $validatedData['form']['gain']['hp_restant'];
